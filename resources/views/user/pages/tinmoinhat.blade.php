...
@section('title')
Tin tức 24h | Tin Mới Nhất
@endsection

@section('main-content')
<div class="breadcrumbs column">
      <p><a href="{{url('/')}}">Home.</a>\\ <span style="color:green;text-shadow:0.5px 0.5px 0.5px blue;">Tin Mới Nhất</span>.</p>
</div>
             ...
             <!-- PHÂN TRANG (ko dùng framework laravel)-->
              <div class="pager">
                            <ul>
                              ...
                              <li><a href="{{ $all_news->url($all_news->currentPage() -1)}}" class="first-page"></a></li>
                              @endif
                              @if($all_news->lastPage()>1)
                              @for($i=1;$i<=$all_news->lastPage();$i++)
                                <li><a href="{{$all_news->url($i)}}" @if($all_news->currentPage()==$i)class="active" @endif>{{$i}}</a></li>  
                              @endfor
                              @endif
                              @if($all_news->currentPage()<$all_news->lastPage())
                              <li><a href="{{ $all_news->url($all_news->currentPage() +1)}}" class="last-page"></a></li>
                              @endif
                            </ul>
              </div> 
          </div>
